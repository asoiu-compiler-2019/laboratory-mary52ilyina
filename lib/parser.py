...
Inherits the Tokenizer module and parses the attached file's tokens as they are
...
class Parser(Tokenizer, CodeGenerator):
    """Parser class

    Parses the given source file using the defined language structure.

    Inherits:
        Tokenizer: The lexer component of the compiler.
    ...
    def _parse_assignment_statement(self):
        ...
        index_reg = self.get_register(inc=False)
        ...
        expr_reg = self.get_register(inc=False)
    ...
    def _parse_expression(self):
        ...
        while True:
            operand1 = self.get_register(inc=False)
            ...
            operand2 = self.get_register(inc=False)
    ...
    def _parse_factor(self):
        ...
        if self._accept('symbol', '('):
            ...
            self.generate('R[%d] = (long long int)"%s";' % (self.get_register(), str_val))
        elif self._accept('keyword', 'true'):
            id_type = 'bool'

            self.generate('R[%d] = 1;' % (self.get_register()))
        elif self._accept('keyword', 'false'):
            id_type = 'bool'

            self.generate('R[%d] = 0;' % (self.get_register()))
        elif self._accept('symbol', '-'):
            if self._first_name():
