...
class Tokenizer:
    ...
    def attach_source(self, src_path):
        ...
        if not isfile(src_path):
            raise FileNotFoundError(f'Inputted path: {src_path} is not a file')
    ...
    def next_token(self):
        ...
        if char == '"':
            ...
            self._scan_warning(f'Invalid character "{char}" encountered', hl=self._char_pos-1)
    ...
    def _get_line(self, line_number):
        ...
        """
        if line_number < 0 or  len(self._src) <= line_number:
            raise IndexError(f'Index {line_number} out of range.')
